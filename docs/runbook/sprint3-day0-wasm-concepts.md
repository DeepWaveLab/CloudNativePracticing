...
!!! example "把這個時機放進一個具體的需求裡"
    ...
    **傳統的兩條路,各卡在哪**:
    ...
    - **第二個直覺:把程式碼收進來,包成容器自己跑。** 省掉了網路往返,但換來兩個問題:容器裡的程式能做的事,跟你自己部署的服務一樣多——程式是客戶寫的,出了事卻算在你的環境頭上;而且每個客戶都要常駐一顆容器,幾百個客戶就是幾百顆 pod,養著的卻只是一段幾秒鐘就跑完的規則。

    **改用 wasm**:客戶把規則編譯成 wasm 模組上傳,你的服務在收單流程裡直接載入執行,只授權兩件事——訂單資料進、判定結果出。開檔案?沒授權就做不到。連網路?同樣做不到。幾百個客戶的模組可以載進同一個行程,不用每個客戶開一顆 pod。

    **跟傳統差在哪**:回頭看,前面兩條路卡的其實是同一個兩難——把客戶的邏輯**隔在外面**,安全,但每單多一次網路往返,客戶還得自己養服務;**收進來**,快,但容器裡的程式跟你自己的服務能做的事一樣多,你只能信任它。**wasm 把這個兩難拆掉了:客戶的邏輯直接在你的服務裡面跑,呼叫它跟呼叫自己的程式一樣直接;但它被關在沙箱裡,隔離程度跟放在外面一樣。** 快和隔離,第一次可以同時要。
...
3. **default branch 的最後一筆非機器人 commit,加近 13 週逐週分布**——不要用 GitHub 的 `pushed_at`,它把 dependabot 推到自己分支的自動升級也算進去([地雷 1](#mine-1));總和也會騙人,52 週 481 個 commit 可以最後 7 週全是零,**看連續零,不看總和**。
...
### 地雷 1:`pushed_at` 把 dependabot 分支算成活性 {#mine-1}

**現象**:用 `gh api repos/{r}` 的 `pushed_at` 判斷活性,會看到「上週還有 push」;實際去看,那次 push 是 dependabot 推到自己分支、沒人合併的相依性升級。查證時(2026-08-11)三個 wasm 相關 repo 的 `pushed_at` 與 default branch 最後 commit 相差 7–8 週。

**根因**:`pushed_at` 涵蓋**任何分支**的 push,機器人也算。

**判斷準則**:活性用三個量——default branch 最後 commit、最後一筆非相依性更新的 commit(過濾標題含 `deps`/`bump`)、近 13 週逐週分布。
