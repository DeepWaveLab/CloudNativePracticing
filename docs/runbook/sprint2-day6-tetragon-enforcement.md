...
要補這個洞,選項只有兩個,兩個都很貴:把攔截策略做成叢集範圍(在共用叢集上風險極高,一條走偏的殺人策略會把別人的元件一起帶走),或者不讓任何 pod 拿到特權加 `hostPID`——也就是**這個洞的真正修補位置在 admission 層,不在執行期**。
...
兩邊剛好都是 22 筆。但 **Falco 講的故事是錯的**:它報的是 `bash`,因為 exec 在 `security_bprm_check` 就被殺掉了,`execve` 從來沒有完成,Falco 事件裡的行程名還是舊的那個。**它注意到有事發生、名字取錯、也完全沒有辦法知道兇手是誰**——因為殺人的是另一套軟體,而那件事不是一個 syscall。
...
| 允許清單誤殺(22 次) | 22 筆告警,**行程名報成 `bash`(錯的)**,不知道兇手 | 22 筆事件,**執行檔、動作、策略名全對** |
...
- **叢集範圍的攔截策略完全沒有做。** 這座叢集同時跑著別的課程的元件,一條走偏的叢集範圍殺人策略會把它們一起帶走,所以步驟 4 提到的那個補法只有推論,沒有實測。
