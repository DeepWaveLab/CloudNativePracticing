...
接著建叢集。原本的計畫是「2 台 spot」,而那個計畫在第一行指令就撞牆了([地雷 1](#mine-1)):
...
這座叢集是今天新建的,**不在既有的自動關機排程裡**。忘記收工的話它會整夜以每小時的價格計費。

原本的計畫是「兩個節點池都縮到 0」,而那個計畫被 API 退件([地雷 11](#mine-11))。所以只剩 `az aks stop`——**而那正好是一件沒驗證過的事**:BYOCNI 叢集停機再開機之後,CNI 會不會自己回來?

既然變成必經之路,就當場驗證,而不是留給隔天早上:
...
**後果**:「縮容到零」這個收工方式在這座叢集上不存在,只剩 `az aks stop`。而那正好是一件沒驗證過的事(BYOCNI 叢集停機再開,CNI 會不會自己回來),於是**原本想迴避的未驗證項目變成必經之路**。

**處理方式**:既然必經,就當場驗證完整的停機、開機、驗收、再停機,而不是把風險留給隔天。
